...
#1.) What questions would a customer want answered if they were visiting a car dealership website? 
#2.) What problems would a customer want a car web app to solve? 
#3.) What is a user usally looking for when searching for a car? 

#1a.) What cars are avaialble? 
#1b.) What's the average price for a certain model? 
#1c.) What's your cars trade in value?
#1d.) What vehicles are available within a certain price range? 
#1f.) What vehicle types are available? ie: Pick-Up's, MiniVans, SUV's, etc
#1g.) Vehicle Type by Manufacturer (TripleTen sylabus)
#1h.) Compare price distrobution between manufacturers (TripleTen Sylabus)

#Lets start with the functions already made for us 1g & 1h 
